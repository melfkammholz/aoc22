...
solve :: [Jet] -> Int -> (Int, [Jet])
solve ps n = go 0 Vector.empty (cycle [minBound..]) (cycle ps) n
  where
    go h v _      ps 0 = (h, ps)
    go h v (s:ss) ps n =
      let v'             = insert h s v                            -- insert space if necessary
          o              = Vector.length v' - h - 3 - height s - 1 -- adjust insertion height
          (v'', ps', h') = sim' v' 2 o s ps                        -- simulate shape moving
...
main = do
  ...
  case parse parseJetPattern "" contents of
    ...
    Right ps -> do
      let n          = 1000000000000
      let (a, b)     = (1085, 1705) -- looked at the output
      let (r, ps')   = solve ps $ a
      let (r', ps'') = solve ps' $ b
      let (r'', _)   = solve ps'' ((n - a) `mod` b)
      print $ ((n - a) `div` b) * r' + r'' + r
      -- 1553665689155
